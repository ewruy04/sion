...
function shapesToPolyObjs(saved, ellipseSegments=72){
  ...
  for(const sh of saved.shapes){
    ...
    // 펜툴 곡선도 저장 시에는 points 배열로 들어온다고 가정
  }
  return out;
}
...
let footprintTemplates = []; // Only outline will be pushed
let floorWorldY = 0;

// Outline presence flags
...
(function initLoadedPolys(){
  ...
  window.loadedPolyObjs = loadedPolys; // for blocking/culling
...
function buildOnePolygon(polyObj, baseHeight, floors, color = 0x9bb0c1){
  ...
  const chainKey = polyObj.id || THREE.MathUtils.generateUUID();
  oneGroup.userData.polyObj  = polyObj;
  oneGroup.userData.chainKey = chainKey;
  oneGroup.name = `oneGroup_${chainKey}`;
  ...
  for (const e of edges){
    const { i, a, b, len } = e;
    const dx=b.x-a.x, dz=b.y-a.y;              // 에지 벡터 in XZ
    const midX=(a.x+b.x)/2, midZ=(a.y+b.y)/2;
    const ang=Math.atan2(dz,dx);

    // 외측 법선(nx,nz) 고정: CCW → 왼쪽(-dz,+dx), CW → 오른쪽(+dz,-dx)
    let nx, nz;
    if (isCCW){
      nx = -dz / (len || 1);  nz =  dx / (len || 1);
    }else{
      nx =  dz / (len || 1);  nz = -dx / (len || 1);
    }
  ...
  // Footprint template — ONLY outline (or if no outline exists, use first)
...
/* ──────────────────────────────────────────────────────────
 * Texture: dblclick
 *  - Any shape: wrap its full wall-chain
 *  - Overlap culling vs. other shapes (excluding outline & self)
 *  - UV repack with seam rotation to start on first allowed range
 * ────────────────────────────────────────────────────────── */
const raycaster = new THREE.Raycaster();
const mouse = new THREE.Vector2();
const fileInput = document.getElementById('fileInput');
let pending = null; // { chain, parentGroup }
...
renderer.domElement.addEventListener('dblclick', (e)=>{
  ...
  pending = { chain, parentGroup: oneGroup };
  fileInput.value = '';
  fileInput.click();
});
...
fileInput?.addEventListener('change', (e)=>{
  ...
  new THREE.TextureLoader().load(url, (baseTex)=>{
    ...
    const oneGroup = pending.parentGroup;
    const shapeObj = oneGroup.userData?.polyObj;
    ...
    let allowed = complementRanges(blocked); // [[u0,u1], ...]

    // rotate seam so first allowed starts at u=0
    let offset=0;
    if (allowed.length){
      offset = allowed[0][0];
    ...
    // ★ 외측 법선(nx,nz) 사용으로 래핑 오프셋 방향 안정화
    ...
    // remove previous wrap and hide walls
    const prev = oneGroup.getObjectByName('wrapGroup_chain');
    if (prev){ prev.userData?.hiddenWalls?.forEach(w=>w.visible=true); disposeObject3D(prev); oneGroup.remove(prev); }
    ...
    wrap.name = 'wrapGroup_chain';
    wrap.renderOrder = 10;
    oneGroup.add(wrap);

    wrap.userData.hiddenWalls = chain.slice();
    chain.forEach(w => (w.visible = false));

    // Build curved mesh along allowed ranges
    const positions=[], uvs=[], indices=[];
    const TARGET_STEP = 0.6; // world units per segment
    const eps = 0.004;
    let vertBase=0;

    // 필요 시 U 반전 옵션 (기본 false)
    const FLIP_U = false;
    ...
    for(let i=0;i<allowed.length;i++){
      ...
      for(let k=0;k<sampleN;k++){
        const t = k/(sampleN-1);
        const us = sa + usLen*t;   // rotated space
        const u0 = unshiftU(us);   // original u
        const {x,z,nx,nz} = posOnChain(u0);

        const ox = x + nx*eps, oz = z + nz*eps; // ★ 항상 외측으로 미세 오프셋
        positions.push(ox,0,oz); positions.push(ox,H,oz);

        let uPacked = packedU(us);
        if (FLIP_U) uPacked = 1 - uPacked;
        uvs.push(uPacked,0); uvs.push(uPacked,1);
    ...
    curved.name='curved_single_mesh';
    wrap.add(curved);
